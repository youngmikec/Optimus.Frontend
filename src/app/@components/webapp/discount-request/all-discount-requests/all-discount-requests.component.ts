...
@Component({
  selector: 'app-all-discount-requests',
  templateUrl: './all-discount-requests.component.html',
  styleUrls: ['./all-discount-requests.component.scss'],
})
export class AllDiscountRequestsComponent implements OnInit, OnDestroy {
  ...
  getDiscountRequestList(
    ...
    this.store.dispatch(
      DiscountActions.GetAllDiscountRequest({
        payload: { skip: skip, take: take, status: 0 },
      })
    );
  }
