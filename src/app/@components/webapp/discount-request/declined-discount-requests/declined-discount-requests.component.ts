...
@Component({
  selector: 'app-declined-discount-requests',
  templateUrl: './declined-discount-requests.component.html',
  styleUrls: ['./declined-discount-requests.component.scss'],
})
export class DeclinedDiscountRequestsComponent implements OnInit, OnDestroy {
  ...
  getDiscountRequestList(
    ...
    this.store.dispatch(
      DiscountActions.GetAllDiscountRequest({
        payload: { skip: skip, take: take, status: 3 },
      })
    );
  }
