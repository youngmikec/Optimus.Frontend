...
@Component({
  selector: 'app-pending-discount-requests',
  templateUrl: './pending-discount-requests.component.html',
  styleUrls: ['./pending-discount-requests.component.scss'],
})
export class PendingDiscountRequestsComponent implements OnInit, OnDestroy {
  ...
  getDiscountRequestList(
    ...
    this.store.dispatch(
      DiscountActions.GetAllDiscountRequest({
        payload: { skip: skip, take: take, status: 1 },
      })
    );
  }
